...
impl Storage {
    ...
    pub(crate) fn const_set(&mut self, v: Scalar, l: &Layout) -> Result<()> {
        match self {
            ...
            #[cfg(feature = "webgpu")]
            Storage::WebGpu(storage) => storage.const_set(v, l),
        }
    }
    ...
    pub(crate) fn apply_op1(&self, l: &Layout, c: &dyn CustomOp1) -> Result<(Self, Shape)> {
        match self {
            ...
            #[cfg(feature = "webgpu")]
            Self::WebGpu(_storage) => {
                crate::bail!("WebGPU backend does not support custom ops yet")
            }
        }
    }
    ...
    pub(crate) fn apply_op2(
        ...
        match (self, t2) {
            ...
            #[cfg(feature = "webgpu")]
            (Self::WebGpu(_s1), Self::WebGpu(_s2)) => {
                crate::bail!("WebGPU backend does not support custom ops yet")
            }
            _ => unreachable!(),
        }
    ...
    pub(crate) fn apply_op3(
        ...
        match (self, t2, t3) {
            ...
            #[cfg(feature = "webgpu")]
            (Self::WebGpu(_s1), Self::WebGpu(_s2), Self::WebGpu(_s3)) => {
                crate::bail!("WebGPU backend does not support custom ops yet")
            }
            _ => unreachable!(),
        }
    ...
    pub(crate) fn inplace_op1(&mut self, l: &Layout, c: &dyn InplaceOp1) -> Result<()> {
        match self {
            ...
            #[cfg(feature = "webgpu")]
            Self::WebGpu(_storage) => {
                crate::bail!("WebGPU backend does not support inplace ops yet")
            }
        }
    }
